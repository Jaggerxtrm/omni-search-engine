...
class VaultIndexer:
    ...
    def index_vault(self, force: bool = False) -> IndexingResult:
        """
        ...
        for i, file_path in enumerate(markdown_files, 1):
            ...
            try:
                ...
                # Index the file
                chunks_count = self.index_single_file(file_path)
    ...
    def move_file(self, src_path: Path, dest_path: Path) -> bool:
        """
        ...
        if not old_data['ids']:
            ...
            self.index_single_file(dest_path)
            return False
    ...
    def _cleanup_orphans(self, current_files: List[Path]) -> int:
        ...
        current_paths = {
            get_relative_path(f, self.vault_path)
