...
async def main():
    ...
    service = EmbeddingService(
        api_key=api_key, model="text-embedding-3-small", batch_size=100
    )
    ...
    if dimension == 1536:
        print("  ✓ Correct dimension for text-embedding-3-small (1536)")
    else:
        print(f"  ✗ Unexpected dimension: {dimension} (expected 1536)")

    if len(embeddings) == len(texts):
        print(
            f"  ✓ Batch size matches: {len(embeddings)} embeddings for {len(texts)} texts"
        )
    else:
        print(f"  ✗ Batch size mismatch: {len(embeddings)} != {len(texts)}")

    if all(len(emb) == dimension for emb in embeddings):
        print("  ✓ All embeddings have consistent dimension")
    else:
