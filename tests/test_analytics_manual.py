...
def get_vault_structure_impl(root_path: Optional[str] = None, depth: int = 2) -> Dict[str, Any]:
    config = MockConfig()
...
def search_notes_impl(pattern: str, root_path: Optional[str] = None, max_results: int = 50) -> List[str]:
    config = MockConfig()
    base_path = config.vault_path
