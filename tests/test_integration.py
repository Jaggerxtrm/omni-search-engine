...
result = indexer.index_vault(force=True)
...
if stats['total_chunks'] == result.chunks_created:
    ...
    print(f"✗ Chunk count mismatch: {stats['total_chunks']} vs {result.chunks_created}")
print()
...
query1_embedding = embedding_service.embed_single(query1)
...
for i, (doc, meta, dist) in enumerate(zip(results1['documents'], results1['metadatas'], results1['distances'])):
    similarity = 1 - dist
...
query2_embedding = embedding_service.embed_single(query2)
...
for i, (doc, meta, dist) in enumerate(zip(results2['documents'], results2['metadatas'], results2['distances'])):
    similarity = 1 - dist
    print(f"Result {i+1} (similarity: {similarity:.3f}):")
    print(f"  File: {meta['file_path']}")
    print(f"  Header: {meta['header_context']}")
...
query3_embedding = embedding_service.embed_single(query3)
...
for i, (doc, meta, dist) in enumerate(zip(results3['documents'], results3['metadatas'], results3['distances'])):
    similarity = 1 - dist
    print(f"Result {i+1} (similarity: {similarity:.3f}):")
    print(f"  File: {meta['file_path']}")
    print(f"  Header: {meta['header_context']}")
...
result2 = indexer.index_vault(force=False)
...
gold_file.write_text(gold_file.read_text() + "\n\n## New Section\n\nThis is new content.")

result3 = indexer.index_vault(force=False)
...
print("SUMMARY")
print("=" * 80)
print(f"✓ Test vault created with 3 markdown files")
print(f"✓ Initial indexing: {result.notes_processed} files, {result.chunks_created} chunks")
print(f"✓ Semantic search correctly finds relevant content")
print(f"✓ Incremental indexing skips unchanged files")
print(f"✓ Incremental indexing detects and reprocesses modified files")
print()
print("All integration tests passed!")
print()
print(f"Test directories (can be deleted):")
